...
int main(int argc, char* argv[]) {
    if (argc < 7) {
        std::cerr << "Usage: " << argv[0]
                  << " <tsp_file> <tabu_size> <neighbor_sample> <max_no_improve> <n_trials> <output_prefix>\n";
        return 1;
    }
    std::string fpath   = argv[1];
    int tabu_size       = std::stoi(argv[2]);
    int neighbor_sample = std::stoi(argv[3]);
    int max_no_improve  = std::stoi(argv[4]);
    ...
    TSParams p{tabu_size, neighbor_sample, max_no_improve};
